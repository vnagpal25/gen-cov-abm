...
class NewTransmission(SubstepTransitionMessagePassing):
    ...
    def get_stage_proportions(self, t,  current_stages):
        ...
        csv_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'washtenaw_info.csv')
        ...
        if t == 0:
            ...
            with open(csv_path, mode='w', newline='') as file:
                ...
                writer.writerow(["t", "susceptible", "exposed", "infected", "recovered", "dead"])
    ...
    def get_infected_time(self, agents_stages):
        ...
        agents_infected_time[agents_stages == 2] = -1*self.EXPOSED_TO_INFECTED_TIME  # -1*exposed_to_infected_time
    ...
    def get_mean_agent_interactions(self, agents_ages):
        ...
        agents_mean_interactions = 0 * torch.ones(size=agents_ages.shape)  # shape: (num_agents)
    ...
    def get_next_stage_time(
        ...
        agents_next_stage_time[(agents_stages==self.RECOVERED_VAR)*(agents_stages == t)] = t+infected_to_recovered_times   # they go back to susceptible
    ...
    def forward(self, state, action=None):
        ...
        initial_infection_rate = self.config['simulation_metadata']['INITIAL_INFECTION_RATE']
        ...
        all_node_attr = (
            torch.stack(
                (
                    ...
                    agents_mean_interactions_split.to(self.device),  # 4 *agents_mean_interactions_split,
                    torch.unsqueeze(
        ...
        agents_data = Data(
            all_node_attr, edge_index=all_edgelist, edge_attr=all_edgeattr, t=t, num_nodes=num_nodes)
